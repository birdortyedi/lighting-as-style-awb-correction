...
class SyntheticDataset(Dataset):
    ...
    def __getitem__(self, index):
        filename = path.join(self.base_dir, "scene_" + str(index + 1) + "_{}_CS.jpg")
        gt_filename = path.join(self.gt_dir, "scene_" + str(index + 1) + "_G_AS.jpg")
        ...
        d_img_R = ops.to_tensor(d_img_R, dims=3)
        s_img_R = ops.to_tensor(s_img_R, dims=3)
        t_img_R = ops.to_tensor(t_img_R, dims=3)

        if f_img is not None:
            f_img_R = ops.to_tensor(f_img_R, dims=3)
        if c_img is not None:
            c_img_R = ops.to_tensor(c_img_R, dims=3)

        img = torch.cat((d_img_R, s_img_R, t_img_R), dim=0)
        if f_img is not None:
            img = torch.cat((img, f_img_R), dim=0)
        if c_img is not None:
            img = torch.cat((img, c_img_R), dim=0)
        ...
        if c_img is not None and f_img is not None:
            return {'gt': gt_img, 'image': img, 'fs_d_img': d_img, 'fs_s_img': s_img, 'fs_t_img': t_img, 'fs_f_img': f_img, 'fs_c_img': c_img, 'filename': filename}
        elif c_img is not None:
            return {'gt': gt_img, 'image': img, 'fs_d_img': d_img, 'fs_s_img': s_img, 'fs_t_img': t_img, 'fs_c_img': c_img, 'filename': filename}
        elif f_img is not None:
            return {'gt': gt_img, 'image': img, 'fs_d_img': d_img, 'fs_s_img': s_img, 'fs_t_img': t_img, 'fs_f_img': f_img, 'filename': filename}
        else:
            return {'gt': gt_img, 'image': img, 'fs_d_img': d_img, 'fs_s_img': s_img, 'fs_t_img': t_img, 'filename': filename}
...
class CubeWBDataset(Dataset):
    ...
    def __getitem__(self, index):
        filename = path.join(self.base_dir, str(index+1) + "_{}.png")
        ...
        t_img = ops.imread(filename.format("T"))
        f_img = ops.imread(filename.format("F")) if 'F' in self.wb_settings else None
        c_img = ops.imread(filename.format("F")) if 'C' in self.wb_settings else None
    ...
    def __len__(self):
        ...
        return len(listdir(self.base_dir)) // 5
