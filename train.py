...
def train_net(net, device, data_dir, val_dir=None, epochs=140,
  ...
  ''')
    ...
    if optimizer_algo == 'Adam':
        optimizer = optim.Adam(net.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=l2reg)
    elif optimizer_algo == 'AdamW':
        optimizer = optim.AdamW(net.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=l2reg)
...
def get_args():
  ...
  """
    ...
    parser.add_argument('-s', '--patch-size', dest='patch_size', type=int, default=128, help='Size of input training patches')
    parser.add_argument('-b', '--batch-size', metavar='B', type=int, nargs='?', default=8, help='Batch size', dest='batch_size')
    ...
    parser.add_argument('-mvf', '--max-val-files', dest='max_val_files', type=int, default=0, help='max number of validation files; default is 0 which uses all files')
    ...
    parser.add_argument('-vf', '--validation-frequency', dest='val_freq', type=int, default=1, help='Validation frequency.')
    parser.add_argument('-cpf', '--checkpoint-frequency', dest='cp_freq', type=int, default=1, help='Checkpoint frequency.')
    ...
    parser.add_argument('-valdir', '--validation-dir', dest='valdir', default=None, help='Main validation directory')
...
if __name__ == '__main__':
    ...
    net = wb_net.WBnetIFRNet(ps=args.patch_size, device=device, norm=args.norm, inchnls=3 * len(args.wb_settings))
